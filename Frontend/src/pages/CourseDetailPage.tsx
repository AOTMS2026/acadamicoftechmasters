...
} from "./course-detail/CourseOutcomes";
...
export default function CourseDetail() {
  ...
  const customSections = getCourseCustomContent(
    course.title,
    course.category || "",
  );
  const features = getCourseFeatures(course.category || course.title);
  const theme = getCourseTheme(course.category || course.title);
  const toolsData = getCourseToolsAndTechnologies(
    course.title,
    course.category || "",
  );
  const objectives = getCourseObjectives(course.title, course.category || "");
  const capstoneProjects = getCourseCapstoneProjects(
    course.title,
    course.category || "",
  );
  const learningOutcomes = getCourseLearningOutcomes(
    course.title,
    course.category || "",
  );

  const careerRoles = getCourseCareerRoles(course.title, course.category || "");
  const hiringCompanies = getCourseHiringCompanies(
    course.title,
    course.category || "",
  );
  const curriculumData = getCourseCurriculum(
    course.title,
    course.category || "",
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Sticky Section Navigation - Outside container for proper sticky behavior */}
      <div className="sticky top-[5.5rem] md:top-[7.5rem] z-50 bg-slate-50 backdrop-blur-md border-b border-slate-200 shadow-sm py-3">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <CourseLearningOutcomes
              outcomes={learningOutcomes as React.ReactNode[]}
            />
